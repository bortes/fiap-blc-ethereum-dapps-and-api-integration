...
if (window.ethereum) {
    ...
    setTimeout(function(){
        showError('Não foi possível se conectar a rede. Considere utilizar o <a href="https://metamask.io/">MetaMask<a>');
    }, 500);
}
...
window.onload = async () => {
    ...
    setTimeout(function(){
        if (!result) {
            showError('Não foi possível se conectar a rede. Recarregue a página');
        } else {
            showError();
...
function showError(message){
    ...
    if(typeof message != 'string') {
        document.getElementById("erro").className = 'd-none';
        return;
    }
